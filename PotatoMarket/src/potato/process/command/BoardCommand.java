...
public class BoardCommand extends AbsCommand {
	...
	public BoardCommand() {
		command = new HashMap<>();
		command.put(1, new SearchController()); // 상품 검색
		//command.put(2, new ); // 상품 구매
		command.put(3, new UpdateController()); // 게시글 수정
		command.put(4, new DeleteController()); // 게시글 삭제
		command.put(5, new SellHistoryController()); // 내 판매 내역 확인
